...
export default function PrivacyPolicyPage() {
  ...
  const { data: policyData, isLoading: isFetching } =
    useGetPrivacyPolicyQuery(undefined);
  const [updatePrivacyPolicy, { isLoading: isUpdating }] =
    useUpdatePrivacyPolicyMutation();

  // 2. FIXED: Extract the specific 'privacyPolicy' field from the object
  useEffect(() => {
    // Check if data exists
    if (policyData?.data) {
      // The backend response is { data: { privacyPolicy: "<html>..." } }
      // So we access policyData.data.privacyPolicy
      const backendHtml = policyData.data.privacyPolicy;
